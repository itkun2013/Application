...
public class ButtonFloat extends Button {
    ...
    protected void setDefaultProperties() {
        ...
        rippleSize = UiUitls.dpToPx(5, getResources());
    ...
    protected void setAttributes(AttributeSet attrs) {
        ...
        // Set Ripple Color
        // Color by resource
        int rippleColor = attrs.getAttributeResourceValue(KONSUNG, "rippleColor", -1);
        if (rippleColor != -1) {
            setRippleColor(getResources().getColor(rippleColor));
        } else {
            // Color by hexadecimal
            int background = attrs.getAttributeIntValue(KONSUNG, "rippleColor", -1);
            if (background != -1)
        ...
        // Icon of button
        int iconResource = attrs.getAttributeResourceValue(KONSUNG, "iconDrawable", -1);
        if (iconResource != -1)
            drawableIcon = getResources().getDrawable(iconResource);
        final boolean animate = attrs.getAttributeBooleanValue(KONSUNG, "animate", false);
        post(new Runnable() {

            @Override
            public void run() {
                showPosition = ViewHelper.getY(ButtonFloat.this) - UiUitls.dpToPx(24
                        , getResources());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (x != -1) {
            ...
            Rect dst = new Rect(UiUitls.dpToPx(1, getResources()),
                    UiUitls.dpToPx(2, getResources()), getWidth()
                    - UiUitls.dpToPx(1, getResources()), getHeight()
                    - UiUitls.dpToPx(2, getResources()));
